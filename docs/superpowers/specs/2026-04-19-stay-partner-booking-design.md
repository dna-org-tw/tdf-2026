...
- 會員只能以整週為單位訂房。
- 頁面上只有一個合作住宿、只有一種房型（房型詳細介紹：https://taitung.nordenruder.com/en/room-detail/66/）。
- 每位會員最多只能訂 1 間房，但可以在同一流程中預訂多個週次。
- 每間房可入住 1–2 人，但只有主要訂房者必須是會員。
- 每個週次最晚必須在入住日前 3 天完成預訂。
- 對訂房者直接採用合作報價單的折後含稅價格，但所有對外與系統內部執行金額都使用整數 TWD，並對小數採無條件進位。
...
| 預訂截止 | 每個週次最晚需在入住日前 3 個日曆天的 23:59（台灣時間）完成預訂 |
| 對外售價 | 以報價單折後含稅價格為基礎，但對訂房者顯示與同意的價格使用整數 TWD、無條件進位 |
...
3. 每週庫存區
   - 固定 4 個住宿週次
   - 顯示折後含稅且已無條件進位的整數 TWD 價格、房量、目前可訂狀態
   - 若已超過該週入住日前 3 個日曆天的 23:59（台灣時間）截止時間，該週不可再預訂
...
4. 房型資訊
   ...
   - 不可加床 / 不適合 12 歲以下兒童
5. 訂房規則
   - 每個週次最晚需在入住日前 3 個日曆天的 23:59（台灣時間）完成預訂
   - 對訂房者直接顯示折後含稅且無條件進位後的整數 TWD 價格，不再以原價作為主要售價
   - 一般訂房：先驗卡、現場付款、取消 / 未到收整週
   - 招待訂房：需招待碼、免綁卡、免現場付款
   - 改期與轉讓規則
6. FAQ / logistics
   - 地址
   - 英文介面可顯示約略 USD 換算，但需標註僅供參考，正式結算依匯率為準
...
5. 系統驗證：
   - 會員已登入
   - 所選週次仍有名額
   - 當前時間尚未超過各週次的預訂截止時間（入住日前 3 個日曆天的 23:59，Asia/Taipei）
   - 會員未在相同週次持有另一間有效房間
6. 若沒有招待碼：
   - 再次顯示高風險條款
...
候補也遵守同樣的截止規則：某週一旦超過入住日前 3 個日曆天的 23:59，就不再接受新的候補加入，也不再發送新的遞補 offer。
...
- `2026-w1` → 報價單折後價 NT$6,124.80 → 對外使用價 NT$6,125 / 週 / 間（約 US$194.77）
- `2026-w2` → 報價單折後價 NT$4,904.00 → 對外使用價 NT$4,904 / 週 / 間（約 US$155.95）
- `2026-w3` → 報價單折後價 NT$5,025.60 → 對外使用價 NT$5,026 / 週 / 間（約 US$159.82）
- `2026-w4` → 報價單折後價 NT$5,120.00 → 對外使用價 NT$5,120 / 週 / 間（約 US$162.82）

實作上應以無條件進位後的整數 TWD 作為 `price_twd`，並以此作為：

- 訂房頁顯示價格
- 會員確認與同意的價格
- 後續 no-show / cancellation charge 的基準價格

預訂截止時間不另外存欄位，直接由 `starts_on` 推導：

- `booking_deadline_date = starts_on - interval '3 days'`
- 實際截止時刻為 `booking_deadline_date 23:59:59 Asia/Taipei`
- 例如：2026-04-30 入住的週次，最晚預訂時間為 2026-04-27 23:59:59（台灣時間）
- 一旦超過截止時間，該週對會員端視為不可預訂，也不可新加入候補
...
因為產品決策已明確規定所有對外價格採整數 TWD 並無條件進位，住宿模組中的 week price / booked price / no-show charge amount 都應以整數 TWD 保存與執行，不保留小數金額作為實際扣款基準。
...
- 中文版：以折後含稅 TWD 價格為主
- 英文版：以無條件進位後的整數 TWD 為主，USD 僅作 approximate helper text，並加註依匯率為準

## 錯誤處理

- 容量檢查必須在最終寫入時於 server-side 執行，不能只靠頁面載入時的前端資訊。
- 預訂截止時間檢查必須在 server-side 執行，不能只靠前端關閉按鈕。
...
3. 招待碼可建立免費住宿，且無需綁卡。
4. 同一招待碼不可重複使用。
5. 訂房頁對外顯示的是折後含稅、且無條件進位後的整數 TWD 價格，而不是原價。
6. 同一會員不可在同一週持有第二間房。
7. 售罄週次只能加入候補，不能直接成立預約。
8. 超過入住日前 3 個日曆天的 23:59 後，該週不可新訂，也不可新加入候補。
9. 候補 offer 過期後會輪到下一位。
10. 一般轉讓要求受讓人重新綁卡。
11. 招待轉讓不要求受讓人綁卡。
12. 改期能原子性地釋放舊週並保留新週。
13. 高風險條款在所有必要 UI 與 email 節點都有出現。
...
- Stripe supported currencies（含 TWD charge / payout 行為）：https://docs.stripe.com/currencies
