# FAQ & Guide Page Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a `/guide` page with tabbed FAQ + living guide content, and a condensed FAQ section on the homepage.

**Architecture:** New data file `data/guide.ts` holds all bilingual FAQ/guide content. A reusable `Accordion` component handles expand/collapse. The `/guide` page uses tab navigation with URL hash routing. The homepage gets a new `FAQSection` showing 6 key questions as summary cards linking to `/guide`. Guide blocks use static HTML content from our own data file (not user input), rendered safely.

**Tech Stack:** Next.js App Router, React 19, Tailwind CSS 4, Framer Motion, useTranslation hook

**Note on HTML rendering:** The guide blocks contain HTML strings (tables, lists) defined in our own `data/guide.ts` — this is developer-controlled static content, not user input. `dangerouslySetInnerHTML` is safe here as the content is hardcoded in source.

---

### Task 1: Create guide data file (`data/guide.ts`)

**Files:**
- Create: `data/guide.ts`

This is the largest task — it contains all the bilingual content. The data structure drives everything else.

- [ ] **Step 1: Create `data/guide.ts` with the full data structure**
...
Run: `npx tsc --noEmit data/guide.ts` or just ensure no red squiggles. The file is self-contained with no external dependencies.

- [ ] **Step 3: Commit**

```bash
git add data/guide.ts
git commit -m "feat: add bilingual guide/FAQ data"
```
...
A simple, reusable accordion component using Framer Motion.
...
git commit -m "feat: add reusable Accordion component"
```

---

### Task 3: Create `/guide` page

**Files:**
- Create: `app/guide/page.tsx`

- [ ] **Step 1: Create `app/guide/page.tsx`**

Note: Guide blocks use `dangerouslySetInnerHTML` — this is safe because all HTML content is hardcoded in `data/guide.ts` (developer-controlled static content, not user input).
...
git commit -m "feat: add /guide page with tabbed FAQ and living guide"
```

---

### Task 4: Create homepage FAQ section

**Files:**
- Create: `components/sections/FAQSection.tsx`
- Modify: `components/HomeContent.tsx`

- [ ] **Step 1: Create `components/sections/FAQSection.tsx`**
...
- [ ] **Step 2: Add FAQSection to HomeContent.tsx**

In `components/HomeContent.tsx`, add the dynamic import after the AccommodationSection import:
...
Then add `<FAQSection />` between `<AccommodationSection />` and `<TeamSection />` in the JSX:
...
```bash
git add components/sections/FAQSection.tsx components/HomeContent.tsx
git commit -m "feat: add FAQ section to homepage"
```

---

### Task 5: Add Guide link to navigation

**Files:**
- Modify: `data/content.ts` (nav section for both `en` and `zh`)
- Modify: `components/Navbar.tsx` (navLinks array)

- [ ] **Step 1: Add nav labels to `data/content.ts`**

In the `en.nav` object (~line 33 area), add:

```ts
guide: "Guide",
```

In the `zh.nav` object (corresponding location in zh section), add:

```ts
guide: "指南",
```

- [ ] **Step 2: Add guide link to Navbar navLinks array**

In `components/Navbar.tsx`, the `navLinks` array is at ~line 99. Add the guide link. Since `/guide` is a page route (not a hash anchor), it needs to be rendered as a `Link` or an `<a>` with href `/guide`. The simplest approach: add it to the navLinks array but handle the non-hash case.
...
Then update the nav rendering to use `Link` for non-hash hrefs. Replace the desktop nav link rendering (lines ~133-145):
...
Also update `MobileMenu.tsx` if it renders links the same way — it receives `navLinks` as a prop, so the same hash vs route distinction applies. Check `MobileMenu.tsx` and apply the same hash vs route distinction.

- [ ] **Step 3: Commit**

```bash
git add data/content.ts components/Navbar.tsx components/MobileMenu.tsx
git commit -m "feat: add Guide link to navigation"
```
...
- [ ] **Step 2: Run `npm run dev` and test in browser**

Verify:
1. Homepage: FAQ section appears between Accommodation and Team, 6 cards render correctly, CTA links to `/guide`
2. Click a card → navigates to `/guide` with correct tab active
3. `/guide` page: all 8 tabs render, tab switching works, accordion expand/collapse works
4. `/guide` page: guide blocks with HTML tables render correctly
5. URL hash: `/guide#transportation` activates the correct tab
6. Language toggle: both English and Chinese content renders
7. Mobile: tabs are horizontally scrollable, cards stack vertically
8. Navbar: "Guide" / "指南" link appears and navigates to `/guide`
