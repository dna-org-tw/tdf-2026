...
export default function TermsPage() {
  ...
  const sections = (tr?.sections ?? []) as Section[];
      ...
      <article className="container mx-auto px-4 sm:px-6 pt-24 pb-12 max-w-3xl">
        <header className="mb-10">
          <h1 className="text-3xl md:text-4xl font-bold mb-2">
            {tr?.title ?? 'Terms of Service'}
          </h1>
          <p className="text-white/60 text-sm">
            {tr?.lastUpdated ?? 'Last updated: April 2026'}
          </p>
        </header>

        <div className="prose prose-invert prose-sm max-w-none space-y-8">
          {tr?.intro && (
            <p className="text-white/90 leading-relaxed">{tr.intro}</p>
          )}
        ...
        <div className="mt-12 pt-8 border-t border-white/10">
          ...
          >
            {tr?.backToHome ?? 'Back to homepage'}
          </button>
        </div>
      </article>
      <Footer />
