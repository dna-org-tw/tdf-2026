...
本文件記錄了整個網站中已實作的 Meta (Facebook Pixel) 標準事件追蹤。

## 已實現的 Meta 標準事件

**注意**: 我們使用 `CompleteRegistration` 而非 `Subscribe` 來追蹤免費信箱訂閱，因為 `Subscribe` 事件通常用於付費訂閱服務。
...
**位置**: 訂閱表單組件
- **觸發時機**: 使用者成功訂閱免費時事通訊時
- **說明**: 使用 CompleteRegistration 而非 Subscribe，因為我們的信箱訂閱是免費的。Subscribe 事件通常用於付費訂閱。
- **實現位置**:
...
### 4. Subscribe
**說明**: 目前未使用。Subscribe 事件通常用於付費訂閱服務。我們的信箱訂閱是免費的，因此使用 CompleteRegistration 事件。

### 5. InitiateCheckout
**位置**: `components/sections/TicketsSection.tsx`
- **觸發時機**: 使用者點擊票務購買按鈕，開始結帳流程
- **參數**:
  - `content_name`: 票務類型名稱
...
除了 Meta 標準事件外，我們還使用自訂事件來追蹤更細粒度的使用者行為：
...
3. **錯誤處理**: 所有事件追蹤都有錯誤處理，不會影響使用者體驗
4. **資料準確性**: 使用標準事件名稱和參數，確保資料在 Meta 平台中正確識別

## 未來改進建議

1. 考慮新增 `AddToCart` 事件（若未來有購物車功能）
2. 考慮新增 `CompleteRegistration` 事件（用於使用者註冊，若有註冊功能）
3. 考慮新增 `StartTrial` 事件（若有試用功能）
4. 優化事件參數，新增更多上下文資訊（如使用者來源、裝置類型等）
