...
1. **從資料庫獲取**：手動或透過腳本將貼文資料插入到 `award_posts` 表中
2. **從 Instagram API 獲取**：配置 Instagram Graph API 後，系統會自動獲取帶有特定標籤的貼文和 Reels

### 自動抓取 Reels

當使用者存取 award 頁面時，系統會自動呼叫 `/api/award/fetch-reels` API 來：
...
- **互動資料**：likes_count, comments_count, video_play_count, ig_play_count, fb_like_count, fb_play_count, video_duration
- **使用者資訊**：owner_full_name, owner_username, owner_id
...
2. **投票限制**：每個信箱每天只能投票一次，透過 `confirmed_at` 欄位的時間戳來判斷。
